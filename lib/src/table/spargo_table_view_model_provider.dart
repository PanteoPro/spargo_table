...
class SpargoTableViewModelProvider<T> extends InheritedWidget {
  const SpargoTableViewModelProvider({super.key, required this.vm, required super.child});

  final SpargoTableViewModel<T> vm;

  static SpargoTableViewModel<T> of<T>(BuildContext context) {
    final vm = context.dependOnInheritedWidgetOfExactType<SpargoTableViewModelProvider<T>>()?.vm;
